...
function getGroupQuestionRowData_all(campaign_code, quest_num, group_q, q_code, i, ids, cb)
{
    var connection = mysql_dbc.init();

    var query = " call sp_CAMPAIGN_RAWDATA_GROUP_QUESTION_ALL(?, ?, ?, ?)";
    var params = [];
    params.push(campaign_code);
    params.push(quest_num);
    params.push(group_q);
    ...
    mcampaign.get_campaign_select(campaign_code, function(err, rows) {
        ...
        mcampaign.sp_CAMPAIGN_LIST_END_DATA(campaign_code, function(err,rows) {
            ...
            mstatistics.sp_STATISTICS_QUESTION(campaign_code, quest_num, function(err, rows) {
                ...
                res.render('statistics/group', { campaign_code: campaign_code, quest_num:quest_num, campaignData: campaignData, groupList: groupList, questData: questData });
            })
...
router.post("/groupData", function(req, res) {
    ...
    mstatistics.sp_STATISTICS_QUESTION_GROUP_DATA_20180727(campaign_code, group_code, q_code, quest_num, function(err, rows) {
        if(err) {
            console.log(err);
            throw err;
        }
...
router.get("/group/rawdata", function(req, res) {
    ...
    mstatistics.getBrandList(campaign_code, function(err, rows) {
        ...
        for(var i = 0; i<rows.length; i++) {
            ...
            console.log(rows[i].DETAIL_BRAND_CODE);
...
router.get("/group/questionrawdata_all", function(req, res) {
    var campaign_code = req.query.campaign_code;
    var quest_num = req.query.quest_num;
    var group_q = req.query.group_q;
    ...
    mstatistics.getQuestionList(group_q, function(err, rows) {
        ...
        for(var i = 0; i<rows.length; i++) {
            ...
            console.log(campaign_code + "///" + quest_num + "///" + group_q + "///" + rows[i].Q_CODE);
            getGroupQuestionRowData_all(campaign_code, quest_num, group_q, rows[i].Q_CODE, i, rows.length-1, function(data) {
