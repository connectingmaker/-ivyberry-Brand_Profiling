...
var mcontent = {
    sp_CONTENT_SAVE: function(seq, subject, contents, use_yn, callback) {
    ...
    ,get_content_select: function(seq, callback) {
        var connection = mysql_dbc.init();
        var query = " SELECT SEQ,SUBJECT,CONTENTS,USE_YN,INSERT_DATETIME,CASE USE_YN WHEN 'N' THEN '미노출' WHEN 'Y' THEN '노출' END USE_YN_TEXT FROM CONTENT WHERE seq = ? ";
        var params = [];
        params.push(seq);
